class Quota.Views.IndexContactEmail extends Backbone.View

	template: HandlebarsTemplates['contacts/index_contact_email'] #Handlebars.compile($("#quote-template").html()) #JST['quotes/index']
	...
	# 			@model.set("name", "Email", {silent: true})
	# 			
	# 		if @model.isValid(true) && @contact.isValid(true)
	# 			if @model.get("pub_key")
	# 				@model.url = 'emails/'+@model.get("pub_key")
	# 			else
	# 				@model.url = 'emails/'
	# 				
	# 			@model.save(
	...
	# 					error: @handleError
	# 					success: (model) -> 
	# 						self.model.url = 'emails/'+model.get("pub_key")
	# 						self.hideRemove = false
	# 						self.model.trigger('change')
	# 						self.vent.trigger("contact_emails:check_empty")
	# 					silent: true
	# 				}
	# 			)
	...
	destroy: (evt) ->
		...
		@model.remove()
		@vent.trigger('contact_emails:remove_contact_email', {model: @model})
