class Quota.Views.IndexContactUrl extends Backbone.View

	template: HandlebarsTemplates['contacts/index_contact_url'] #Handlebars.compile($("#quote-template").html()) #JST['quotes/index']
	...
	destroy: (evt) ->
		...
		@model.remove()
		@vent.trigger('contact_urls:remove_contact_url', {model: @model})
		...
		@model.save(
			...
			},
			{
				...
				success: (model) -> 
					...
					self.decorateShow()
					self.hideSpinner()
					self.vent.trigger("contact_url:updated", self.model)
			}
		)