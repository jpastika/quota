class Quota.Views.ShowTemplate extends Quota.Views.PageBodyBlock
	...
	initialize: (options)->
		...
		@_itemsView2 = new Quota.Views.ShowTemplateItems({parent_model:@model, parent_child_key: @model.get("pub_key"), collection:@template_items, catalog_items: @catalog_items, vent: @vent})
		
	render: ->
		# $(@el).html(@template({opportunity:@model.toJSON()}))
		# @container_items = @$('#items_container')
		@container_items2 = @$('#items_container')
